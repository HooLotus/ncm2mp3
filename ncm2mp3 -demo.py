...
class MainWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.info_label.setStyleSheet("background-color: white; border: 1px solid #ccc; padding: 10px; font-family: 'Microsoft YaHei'; font-size: 12px;")
    ...
    def add_files(self, files):
        for file in files:
            if file not in self.files_converting and os.path.exists(file):
                ...
                # 创建文件信息标签
                file_info = QLabel(f"文件名: {os.path.basename(file)}路径: {file}")
                file_info.setWordWrap(True)  # 允许文本换行
                file_info.setFixedHeight(40)  # 设置固定高度为60像素
                layout.addWidget(file_info, stretch=7)  # 分配70%的空间
                
                # 创建进度条
                progress_bar = QProgressBar()
                progress_bar.setRange(0, 100)
                progress_bar.setFixedHeight(20)  # 设置进度条高度与文件信息标签一致
                layout.addWidget(progress_bar, stretch=3)  # 分配30%的空间
                
                # 创建删除按钮
                delete_button = QPushButton('删除')
                delete_button.setFixedSize(50, 20)  # 设置按钮大小
